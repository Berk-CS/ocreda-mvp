...
function LibraryNoteCard({ note, query, layout, onClick }: { note: Note; query: string; layout: LibraryLayout | 'search'; onClick: () => void }) {
  ...
  return <button type="button" onClick={onClick} className={`relative overflow-hidden rounded-lg border border-[#e1e1e3] bg-[#f7f7f9] p-2 text-left shadow-[0_3px_12px_rgba(0,0,0,0.15)] transition hover:-translate-y-0.5 hover:border-[#7ca2ff] hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#477bea] ${isGrid ? 'h-[360px]' : isLarge ? 'h-[440px]' : 'h-[360px]'}`}><span className={`block overflow-hidden rounded-md bg-white ${isGrid ? 'h-[280px] px-7 py-7' : isLarge ? 'h-[360px] px-8 py-8' : 'h-[280px] px-7 py-7'}`}><strong className={`block text-base leading-snug text-[#151515] ${isLarge ? 'line-clamp-4 text-lg' : 'line-clamp-3'}`}><HighlightedText text={content.title} query={query} /></strong><span className={`mt-4 block whitespace-pre-wrap text-[15px] leading-[1.55] text-[#777] ${isGrid ? 'line-clamp-[8]' : isLarge ? 'line-clamp-[12]' : 'line-clamp-[8]'}`}><HighlightedText text={content.body || notePreview(note)} query={query} /></span></span><span className="absolute inset-x-5 bottom-4 flex items-center justify-between gap-3 text-xs text-[#aaa]"><span className="min-w-0 truncate">{cleanCategory(note.category) || 'Instant retrieval'}</span><span className="shrink-0">{formatDate(note.created_at)}</span></span><span className="sr-only">{isLarge ? 'large card' : layout} layout</span></button>;
}
